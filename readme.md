...
### 2. 🔍 书签健康检测
* **失效链接扫描**：自动检测无法访问的 HTTP/HTTPS 链接，并支持一键清理（自动跳过浏览器内部协议）。
* **去重**：基于 URL 标准化匹配算法，识别并清理重复书签，默认保留最早添加的记录。

### 3. 🛡️ 安全与开发者友好
* **🐞 调试模式**：开启后仅处理前 20 条书签进行 AI 分类测试，极速反馈并节省 API 消耗额度。
* **两阶段确认**：AI 生成建议后进入预览阶段，用户手动点击“确认整理”后才会真正执行修改。
* **数据备份**：支持标准的 Netscape Bookmark HTML 格式导入与导出，确保数据迁移的安全性。
...
或直接[下载crx](https://github.com/PaiPai121/ReShelf/releases/tag/v1.0.0)
---
